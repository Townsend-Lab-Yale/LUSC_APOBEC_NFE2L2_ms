...
variants <- cancereffectsizeR::select_variants(cesa = analysis, variant_passlist = my_aac_muts, include_subvariants = T)
...
snvs %>%
  mutate(variant_name = stringr::str_replace(string = all_aac, pattern = "_ENSP[:digit:]+",replacement = "")) %>%
  dplyr::select(variant_name,variant_id,trinuc_mut,maf_frequency) %>%
  plyr::rename( c("maf_frequency" = "total tumors with variant")) %>%
  arrange(desc(variant_id)) %>%
...
analysis$mutational_signatures$relative_biological %>% 
